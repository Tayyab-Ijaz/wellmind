...
export default function IndustryEducation() {
  ...
  const capabilities = [
    { icon: <GraduationCap size={22} />, color: B.action,    title: 'Student Dropout Prediction',        desc: 'Early-warning ML models that analyze attendance, LMS engagement, grades, and socioeconomic signals to identify at-risk students 8–12 weeks before dropout, enabling timely intervention.' },
    { icon: <Brain size={22} />,         color: B.primary,   title: 'Adaptive Learning Personalization',  desc: 'AI-driven content sequencing that adjusts lesson difficulty, format, and pacing in real time based on individual mastery levels, learning style signals, and time-on-task data.' },
    { icon: <BarChart3 size={22} />,     color: B.accent,    title: 'Learning Outcome Analytics',         desc: 'Cohort-level dashboards that correlate instructional interventions with measurable learning outcomes — giving educators and administrators evidence-based levers for continuous improvement.' },
    { icon: <Users size={22} />,         color: B.secondary, title: 'Enrollment & Retention Modeling',    desc: 'Predictive models that score prospective students by enrollment likelihood and current students by re-enrollment risk, enabling targeted recruitment and financial aid optimization.' },
    { icon: <Lightbulb size={22} />,     color: B.action,    title: 'Intelligent Tutoring & Chatbots',    desc: 'LLM-powered tutoring assistants fine-tuned on institutional curricula that provide 24/7 Socratic-method support, reducing faculty load while improving student comprehension scores.' },
    { icon: <ShieldCheck size={22} />,   color: B.accent,    title: 'Academic Integrity & Proctoring AI', desc: 'Computer vision and behavioral models for remote exam integrity, plagiarism detection with citation-graph analysis, and AI-generated content identification aligned to institutional policy.' },
  ];

  const useCases = [
    {
      icon: <GraduationCap />, color: B.action,
      title: 'Dropout Prevention System for a University Network',
      desc: 'We built a multi-signal early-warning system for a network of 8 universities ingesting LMS clickstream data, grade trajectories, attendance records, financial aid status, and counselor notes. The model scores all enrolled students weekly and surfaces the top 5% at-risk cohort to advisors through a priority dashboard.',
      metrics: [{ val: '31%', label: 'Dropout Reduction' }, { val: '8wk', label: 'Early Warning' }, { val: '94%', label: 'Model Precision' }],
    },
    {
      icon: <Brain />, color: B.primary,
      title: 'Adaptive Math Platform for a K-12 EdTech Company',
      desc: 'Built a reinforcement-learning-based content sequencer for a K-12 math platform serving 200,000 students. The system continuously estimates each student\'s knowledge state across 480 skill nodes and selects the optimal next exercise to maximize mastery gain per minute of engagement, outperforming static curricula significantly.',
      metrics: [{ val: '28%', label: 'Faster Mastery' }, { val: '200K', label: 'Students Served' }, { val: '4.1×', label: 'Engagement Gain' }],
    },
    {
      icon: <BarChart3 />, color: B.accent,
      title: 'Enrollment Intelligence Platform for a Community College System',
      desc: 'Developed an enrollment propensity model combining demographic data, high-school GPA, FAFSA completion signals, and prior outreach interaction data. The model powers a CRM workflow that prioritizes recruiter outreach and personalizes financial aid messaging, driving a significant increase in enrolled first-generation students.',
      metrics: [{ val: '19%', label: 'Enrollment Lift' }, { val: '3.2×', label: 'Recruiter ROI' }, { val: '22%', label: 'First-Gen Increase' }],
    },
  ];

  const faqs = [
    { q: 'What data sources do you typically work with in education?', a: 'We integrate with LMS platforms (Canvas, Moodle, Blackboard, Google Classroom), SIS systems (Ellucian Banner, PowerSchool, Infinite Campus), CRM tools (Slate, Salesforce Education Cloud), FAFSA / financial aid data, attendance systems, and proctoring platforms via API or data warehouse exports.' },
    { q: 'How do you handle FERPA compliance and student data privacy?', a: 'All student data is processed under a formal data use agreement aligned with FERPA, COPPA (for K-12), and relevant state privacy laws. We work within the institution\'s existing data governance framework, support on-premise or institution-controlled cloud deployments, and never use student data to train models outside the engagement scope.' },
    { q: 'Can your models work across different institutional contexts?', a: 'Yes. We use transfer learning to adapt models trained on similar institutional profiles, requiring as little as one academic year of local data for meaningful accuracy. Models are validated on held-out cohorts before deployment and recalibrated each semester as new data accumulates.' },
    { q: 'Do you integrate with existing LMS and SIS platforms?', a: 'We build certified connectors for Canvas (LTI 1.3), Moodle, Blackboard Ultra, Google Classroom, Ellucian Banner, PowerSchool, and Salesforce Education Cloud. Outputs are delivered as dashboard widgets, API endpoints, or CRM workflow triggers depending on the use case.' },
    { q: 'What does a typical education AI engagement look like?', a: 'We begin with a 2-week data audit assessing LMS data quality, SIS completeness, and historical outcome records. Model development runs 4–8 weeks, followed by a semester-long pilot with a matched control group for impact measurement. Full institutional rollout includes training for advisors, faculty, and administrators.' },
  ];
      ...
      <section style={{
        ...
        <div style={{
          ...
          <div style={{ maxWidth: 1200, width: '100%', display: 'flex', flexDirection: 'column', gap: 'clamp(40px, 5vw, 60px)' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'stretch', gap: 'clamp(18px, 5vw, 44px)' }}>
              ...
              >
                ...
                <motion.p variants={fadeUp} custom={0.15}
                  ...
                  From dropout prediction to adaptive personalization — we build production-grade AI systems for universities, K-12 districts, and EdTech platforms. Privacy-first. Outcome-measured. Institution-ready.
                </motion.p>
              ...
              >
                <img
                  src="https://picsum.photos/seed/educationai/600/600"
            ...
            >
              {[
                { val: '31%',  label: 'Dropout Reduction' },
                { val: '28%',  label: 'Faster Mastery' },
                { val: '19%',  label: 'Enrollment Lift' },
                { val: '200K', label: 'Students Served' },
              ].map((s, i) => (
                <div key={i} style={{
      ...
      <section ref={statsRef} style={{ padding: 'var(--sp-section) 0', position: 'relative', zIndex: 1, background: `linear-gradient(135deg, #170F22 0%, #140B20 100%)`, overflow: 'hidden', display: 'flex', alignItems: 'center' }}>
        ...
        <div style={{ ...PX, position: 'relative', zIndex: 2, width: '100%' }}>
          <div style={{ textAlign: 'center', marginBottom: 'clamp(28px, 4vw, 48px)' }}>
            <SectionBadge dark>Measurable Educational Impact</SectionBadge>
            <h2 className="section-h2 dark" style={{ marginBottom: 0 }}>Numbers That Matter in Education</h2>
          </div>
          <div className="grid-stats">
            <StatCard target={31}  suffix="%"   label="Dropout Reduction"       icon={<Target />}       start={statsVisible} delay={0}   theme={{ color: B.action, bg: B.actionLight }} />
            <StatCard target={94}  suffix="%"   label="Model Precision"         icon={<ShieldCheck />}  start={statsVisible} delay={0.1} theme={{ color: B.secondary, bg: B.secondaryLight }} />
            <StatCard target={28}  suffix="%"   label="Faster Skill Mastery"    icon={<TrendingUp />}   start={statsVisible} delay={0.2} theme={{ color: B.primary, bg: B.primaryLight }} />
            <StatCard target={200} suffix="K+"  label="Students Served"         icon={<Globe />}        start={statsVisible} delay={0.3} theme={{ color: B.accent, bg: B.accentLight }} />
          </div>
        </div>
      </section>
      ...
      <section style={{
        ...
        <div style={{ ...PX, position: 'relative', zIndex: 2 }}>
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} style={{ textAlign: 'center', marginBottom: 'clamp(32px, 5vw, 56px)' }}>
            ...
            <p className="section-lead">End-to-end AI solutions across student success, adaptive learning, and institutional intelligence.</p>
          </motion.div>
      ...
      <section style={{
        ...
        <div style={{ ...PX, position: 'relative', zIndex: 2 }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.55 }} style={{ textAlign: 'center', marginBottom: 'clamp(24px, 4vw, 48px)' }}>
            ...
            <h2 className="section-h2 dark" style={{ fontSize: 'clamp(1.4rem, 3.2vw, 2.4rem)', marginBottom: 12 }}>Built on Institution-Grade Infrastructure</h2>
            <p style={{ color: B.textDarkMid, fontSize: 'clamp(0.95rem, 1.5vw, 1.15rem)', maxWidth: 640, margin: '0 auto' }}>Production tools trusted by universities, K-12 districts, and global EdTech platforms.</p>
          </motion.div>
          <div className="grid-tech">
            {[
              { group: 'ML & NLP',             items: ['PyTorch (Transformers)', 'Scikit-learn', 'Knowledge Tracing (BKT, DKT)', 'SHAP Explainability', 'Reinforcement Learning'], icon: <Brain size={16} />,    color: B.action },
              { group: 'Data & LMS Integration',items: ['Canvas LTI 1.3', 'Moodle API', 'Ellucian Banner', 'PowerSchool', 'xAPI / Caliper Analytics'], icon: <Database size={16} />, color: B.secondary },
              { group: 'Platform & Deployment', items: ['AWS / Azure EDU', 'Docker / K8s', 'FastAPI', 'dbt + Snowflake', 'Apache Kafka'], icon: <Cpu size={16} />,      color: B.primary },
              { group: 'Privacy & Compliance',  items: ['FERPA Aligned', 'COPPA (K-12)', 'GDPR Data Handling', 'On-Premise Deployment', 'Audit Trails & DUA'], icon: <Lock size={16} />,    color: B.accent },
            ].map((col, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'clamp(32px, 5vw, 64px)', alignItems: 'center' }}>
            ...
            <div style={{ flex: '1 1 320px' }}>
              <motion.div initial={{ opacity: 0, x: -28 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.65 }}>
                ...
                <p style={{ color: B.textMid, fontSize: 'clamp(1.2rem, 1.6vw, 1.1rem)', lineHeight: 1.75, marginBottom: 'clamp(20px, 3vw, 32px)' }}>
                  Most AI vendors understand either data science or software. We understand both — plus the pedagogy, institutional dynamics, and regulatory constraints that make education AI uniquely challenging.
                </p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {[
                    { title: 'Privacy-First by Design',       desc: 'FERPA and COPPA compliance baked into the architecture — not bolted on after the fact' },
                    { title: 'Semester-Over-Semester Learning', desc: 'Automated retraining pipelines improve model accuracy with each academic cycle, compounding institutional value over time' },
                    { title: 'Education Domain Fluency',       desc: 'We speak Bloom\'s Taxonomy, IRT, IPEDS, and SIS schema — not just Python and neural networks' },
            ...
            >
              ...
              {[
                ...
                { label: 'Semester Pilot',              duration: '1 semester',color: B.primary,   icon: <BookOpen size={16} /> },
                { label: 'Institution-Wide Rollout',    duration: 'Ongoing',   color: B.accent,    icon: <Activity size={16} /> },
              ].map((step, i) => (
        ...
        <div style={{ ...PX, position: 'relative', zIndex: 2, textAlign: 'center' }}>
          <motion.div initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}>
            ...
            <p className="section-lead" style={{ color: B.textMid, maxWidth: 1200, margin: '0 auto clamp(28px, 4vw, 48px)', lineHeight: 1.75 }}>
              Book a free 30-minute call. Tell us your student success, enrollment, or learning outcome challenge. We'll tell you exactly what's possible — no pitch, no pressure.
            </p>
