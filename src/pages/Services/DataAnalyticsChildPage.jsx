/**
 * DataAnalyticsChildPage.jsx — WellMind Data Solutions
 * Generic child service page for Data Analytics sub-services.
 * Reads :childId from URL params, fetches data from dataAnalyticsChildData.js.
 *
 * Route example:  /services-data-analytics/dashboards
 *                 /services-data-analytics/statistical
 *                 /services-data-analytics/etl
 *
 * Design mirrors ServiceChildPage.jsx exactly.
...
import {
  ...
  Layout, Filter, PieChart, LineChart,
} from 'lucide-react';
...
import { DATA_ANALYTICS_CHILDREN } from './Dataanalyticschilddata';
...
const ICON_MAP = {
  ...
  BarChart3, Settings2, Activity, TrendingUp, RefreshCw,
  Eye, Sparkles, MessageSquare, FileText, Search, Users,
  Layers, GitBranch, Terminal, Layout, Filter, PieChart, LineChart,
};
function Icon({ name, size = 22 }) {
  const C = ICON_MAP[name] || Database;
  return <C size={size} />;
}
...
// ─── MAIN COMPONENT ───────────────────────────────────────────────────────────
export default function DataAnalyticsChildPage() {
  ...
  // Fetch data
  const data = DATA_ANALYTICS_CHILDREN[childId];

  // Redirect if not found
  useEffect(() => {
    if (!data) navigate('/services-data-analytics', { replace: true });
  }, [data, navigate]);
            ...
            <div style={{
              ...
              >
                ...
                <motion.div variants={fadeUp} custom={0} style={{ marginBottom: 12 }}>
                  <Link to="/services-data-analytics" style={{
          ...
          <div style={{ ...PX, maxWidth: 1250, display: 'flex', gap: 'clamp(28px, 5vw, 80px)', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: '1 1 min(100%, 380px)' }}>
              ...
              <p style={{
                ...
                Every engagement delivers production-ready code, dashboards, and documentation that your team can maintain and extend independently.
              </p>
